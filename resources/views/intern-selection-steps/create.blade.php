    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <h6 class="m-0 font-weight-bold text-primary">Create Intern Selection Step</h6>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('internship-offering.selection-steps.index', ['id' => $internPositionBatch->id]) }}"
                            class="{{ request()->routeIs('internship-offering.selection-steps.index') ? 'active' : '' }}">
                            Selection Steps
                        </a>
                    </li>
                    <li class="breadcrumb-item">
                        <a href="{{ route('internship-offering.selection-steps.create', ['id' => $internPositionBatch->id]) }}"
                            class="{{ request()->routeIs('internship-offering.selection-steps.create') ? 'active' : '' }}">
            ...
            <div class="card-body">
                <div class="alert alert-info mb-4">
                    ...
                    <strong>{{ \Carbon\Carbon::parse($internPositionBatch->start_date_intern_position_batches)->format('d M Y') }}</strong>
                    sampai
                    <strong>{{ \Carbon\Carbon::parse($internPositionBatch->end_date_intern_position_batches)->format('d M Y') }}</strong>.
                </div>
                <form method="POST"
                    action="{{ route('internship-offering.selection-steps.store', $internPositionBatch->id) }}">
                    @csrf
                        ...
                        <select name="selection_step_id" id="selection_step_id"
                            ...
                            @foreach ($selectionStep as $step)
                                <option value="{{ $step->id }}"
                                    {{ old('selection_step_id') == $step->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="step_order_intern_selection_steps"
                            id="step_order_intern_selection_steps" value="{{ old('step_order_intern_selection_steps') }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="is_mondatory_intern_selection_steps" id="is_mondatory_intern_selection_steps"
                            ...
                            <option value="1"
                                {{ old('is_mondatory_intern_selection_steps') === '1' ? 'selected' : '' }}>Yes</option>
                            <option value="0"
                                {{ old('is_mondatory_intern_selection_steps') === '0' ? 'selected' : '' }}>No</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="is_invitation_only_intern_selection_steps"
                            ...
                            <option value="1"
                                {{ old('is_invitation_only_intern_selection_steps') === '1' ? 'selected' : '' }}>Yes
                            </option>
                            <option value="0"
                                {{ old('is_invitation_only_intern_selection_steps') === '0' ? 'selected' : '' }}>No
                            </option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description_intern_selection_steps" id="description_intern_selection_steps" rows="4"
                            ...
                            placeholder="Enter description...">{{ old('description_intern_selection_steps') }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="estimated_start_date_intern_selection_steps"
                            ...
                            value="{{ old('estimated_start_date_intern_selection_steps') }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="estimated_end_date_intern_selection_steps"
                            ...
                            value="{{ old('estimated_end_date_intern_selection_steps') }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="status_intern_selection_steps" id="status_intern_selection_steps"
                            ...
                            <option value="active"
                                {{ old('status_intern_selection_steps') == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="inactive"
                                {{ old('status_intern_selection_steps') == 'inactive' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <a class="btn btn-secondary"
                                href="{{ route('internship-offering.selection-steps.index', ['id' => $internPositionBatch->id]) }}">Cancel</a>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Logic to sync is_invitation_only with is_mandatory
            $('#is_mondatory_intern_selection_steps').on('change', function() {
                let isMandatory = $(this).val();

                if (isMandatory === '1') {
                    // Mandatory = Yes → Invitation Only = No (readonly)
                    ...
                    // Mandatory = No → Invitation Only = Yes (readonly)
                    $('#is_invitation_only_intern_selection_steps')
                        .val('1')
                        .prop('readonly', true)
                        .css('pointer-events', 'none')
            ...
            // Run once on page load to sync with old() or default value
